...
fn test_elf_parse() {
    ...
    if let Some(elf) = data.get("elf").and_then(|v| v.as_table()) {
        for (key, value) in elf {
            ...
            assert_eq!(elf.program_headers[0].p_type.value, p1_p_type, "program header 0.p_type doesn't match");
            assert_eq!(elf.program_headers[0].p_flags.value, p1_p_flags, "program header 0.p_flags doesn't match");
            assert_eq!(elf.program_headers[0].p_offset.value, p1_p_offset, "program header 0.p_offset doesn't match");
            assert_eq!(elf.program_headers[0].p_vaddr.value, p1_p_vaddr, "program header 0.p_vaddr doesn't match");
            assert_eq!(elf.program_headers[0].p_paddr.value, p1_p_paddr, "program header 0.p_paddr doesn't match");
            assert_eq!(elf.program_headers[0].p_filesz.value, p1_p_filesz, "program header 0.p_filesz doesn't match");
            assert_eq!(elf.program_headers[0].p_memsz.value, p1_p_memsz, "program header 0.p_memsz doesn't match");
            assert_eq!(elf.program_headers[0].p_align.value, p1_p_align, "program header 0.p_align doesn't match");

            assert_eq!(elf.section_headers[0].sh_type.value, sh1_sh_type, "section header 0.sh_type doesn't match");
            assert_eq!(elf.section_headers[0].sh_flags.value, sh1_sh_flags, "section header 0.sh_flags doesn't match");
            assert_eq!(elf.section_headers[0].sh_addr.value, sh1_sh_addr, "section header 0.sh_addr doesn't match");
            assert_eq!(elf.section_headers[0].sh_offset.value, sh1_sh_offset, "section header 0.sh_offset doesn't match");
            assert_eq!(elf.section_headers[0].sh_size.value, sh1_sh_size, "section header 0.sh_size doesn't match");
            assert_eq!(elf.section_headers[0].sh_link.value, sh1_sh_link, "section header 0.sh_link doesn't match");
            assert_eq!(elf.section_headers[0].sh_info.value, sh1_sh_info, "section header 0.sh_info doesn't match");
            assert_eq!(elf.section_headers[0].sh_addralign.value, sh1_sh_addralign, "section header 0.sh_addralign doesn't match");
            assert_eq!(elf.section_headers[0].sh_entsize.value, sh1_sh_entsize, "section header 0.sh_entsize doesn't match");
        }
    }
}