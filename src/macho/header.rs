...
impl MachHeader {
    pub fn parse(buffer: &[u8]) -> Result<Self, errors::FileParseError> {
        ...
        let file_type   = Field::new(u32::from_le_bytes(buffer[12..16].try_into().unwrap()), 12, 4);
        let ncmds       = Field::new(u32::from_le_bytes(buffer[16..20].try_into().unwrap()), 16, 4);
        let sizeofcmds  = Field::new(u32::from_le_bytes(buffer[20..24].try_into().unwrap()), 20, 4);
        let flags       = Field::new(u32::from_le_bytes(buffer[24..28].try_into().unwrap()), 24, 4);
        
        let reserved: Option<Field<u32>> = if buffer.len() >= 32 {
            Some(Field::new(u32::from_le_bytes(buffer[28..32].try_into().unwrap()),28,4))
        } else {
            None
        };
